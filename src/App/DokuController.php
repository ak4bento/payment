...
class DokuController extends Controller {
	...
	function __construct() {
		$this->default_payment_channel      		= config('payment.doku.DEFAULT_PAYMENT_CHANNEL');
		$this->table_order                  		= config('payment.doku.TABLE_ORDER');
		$this->table_field_no_order         		= config('payment.doku.TABLE_FIELD_NO_ORDER');
		$this->table_field_amount           		= config('payment.doku.TABLE_FIELD_AMOUNT');
		$this->table_field_customer_name    		= config('payment.doku.TABLE_FIELD_CUSTOMER_NAME');
		$this->table_field_customer_phone   		= config('payment.doku.TABLE_FIELD_CUSTOMER_PHONE');
		$this->table_field_customer_email   		= config('payment.doku.TABLE_FIELD_CUSTOMER_EMAIL');
		$this->table_field_customer_address 		= config('payment.doku.TABLE_FIELD_CUSTOMER_ADDRESS');
		$this->table_field_payment_date 			= config('payment.doku.TABLE_FIELD_PAYMENT_DATE');
		$this->table_field_payment_status 			= config('payment.doku.TABLE_FIELD_PAYMENT_STATUS'); 
		$this->table_field_payment_channel 			= config('payment.doku.TABLE_FIELD_PAYMENT_CHANNEL');		
		$this->table_field_payment_approval_code	= config('payment.doku.TABLE_FIELD_PAYMENT_APPROVAL_CODE');
		$this->table_field_payment_session_id		= config('payment.doku.TABLE_FIELD_PAYMENT_SESSION_ID');
		$this->shared_key                   		= config('payment.doku.SHARED_KEY');
		$this->mall_id                      		= config('payment.doku.MALL_ID');
		$this->currency                     		= config('payment.doku.CURRENCY');
		$this->product_name_format  				= config('payment.doku.PRODUCT_NAME_FORMAT');
		$this->show_doku_success_page 				= config('payment.doku.SHOW_DOKU_SUCCESS_PAGE');
		$this->show_finish_page 					= config('payment.doku.SHOW_FINISH_PAGE');
		$this->your_own_finish_page 				= config('payment.doku.YOUR_OWN_FINISH_PAGE');
		$this->payment_available 					= config('payment.doku.AVAILABLE_PAYMENT_CHANNEL');
	...
	public function index() {
		...
		if(config('payment.doku.LIVE_MODE') == TRUE) {
		...
		if(!Request::get('payment_channel')) {
			$this->action = "payment_channel";
			// return view('Akill\Payment::payment_channel',$data);
			$this->result = $data;
			return $this;
		}
		
		if($this->invoice['payment_channel'] == '02') {
			$this->action = "payment_form_mandiri_clickpay";
			// return view('Akill\Payment::payment_form_mandiri_clickpay',$data);
		}elseif ($this->invoice['payment_channel'] == '05') {
			$this->action = "payment_form_permata";
			// return view('Akill\Payment::payment_form_permata',$data);
		}else{
			$this->action = "payment_form";
			// return view('Akill\Payment::payment_form',$data);
		}
		$this->result = $data;
		return $this;
	}
